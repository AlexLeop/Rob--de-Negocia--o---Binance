    ...
    while True:
        try:
            ...
            if abs(sig['z_score']) > z_limit or peak_z_score != 0.0:
                # Calcula Lucro Bruto Esperado (Z-score * Desvio Padrão do Spread). 
                expected_profit = abs(sig['z_score']) * sig.get('std', 0.0)
                if expected_profit < 0.015:
                    print(f"⚠️ [Par {pair_idx}] Aguardando: Spread muito estreito (Lucro Esperado: {expected_profit*100:.2f}% < 1.50% Min).")
                    peak_z_score = 0.0
                    await asyncio.sleep(5); continue
